...
let askQuestion =(rand:number):number=>{
    for(let i=0;i<3;i++){
      ...
      if(value==rand){
        ...
        break;
          ...
          if(i<=1){console.log(` ${chalk.yellow("yours guess is wrong, plz try again")}`)};
        }
    }
 if(grade==true){return 10;}
 else {return 0;}
}
...
function again(){
    let againV=prompt((chalk.blue(`do you want to play again press "y" or "n"     `)));
      ...
      else { console.log (` ${chalk.red(`invalid : plz enter the correct word`)}`);
        again();
         }
  
}

function scoreCal(val:number){
    score=score+val;
}
